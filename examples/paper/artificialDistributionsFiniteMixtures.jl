...
savefig(fig,"Plots/Finite_Noise.pdf")
...
savefig(fig,"Plots/Finite_Target.pdf")
...
savefig(fig,"Plots/Finite_Convolution.pdf")
...
resultsNoise = 0
try
    global resultsNoise = CSV.read("Tables/Finite_Noise.csv",DataFrame)
catch
...
try
    global resultsDeconvolution = CSV.read("Tables/Finite_Deconvolution.csv",DataFrame)
catch
        ...
        for (i,dn) in pairs(dNoise)
            ...
            for (j,dt) in pairs(dTarget)
                ...
                if size(resultsDeconvolution[(resultsDeconvolution[!,"Size"].==N) .& 
                    ...
                    fitDeconvolution = finiteGaussianMixtureDeconvolution(c,fitNoise,k=4)
