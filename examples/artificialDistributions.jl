...
resultsNoise = 0
try
    global resultsNoise = CSV.read("Noise.csv",DataFrame)
catch
...
try
    global resultsDeconvolution = CSV.read("Deconvolution.csv",DataFrame)
catch
...
# Plots Conv Bayes
p1 = plot([0,1],[0,1],color="black",label="")
color = [Dict([100=>"lightblue",1000=>"blue",3000=>"darkblue"])[i] for i in resultsDeconvolution[!,"Size"]]
style = [Dict([10=>:square,1=>:circle])[i] for i in resultsDeconvolution[!,"Ratio"]]
scatter!(p1,resultsDeconvolution[!,"MIO_mean"],resultsDeconvolution[!,"MIO_Convolution_mean"],color=color,markershape=style,xlabel="MIO_Bayesian_Deconvolution",ylabel="MIO_Bayesian_Convolution",label="")
xlims!(0,1)
ylims!(0,1)
